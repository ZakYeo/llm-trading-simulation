...
export class OpenAiAgentSystemContextBuilder {
  ...
  addMarketContextSummary() {
    ...
    this.segments.push(
      `Market context: ${visibleOpportunities} ${selfOpenPositions} ${primaryCounterpartyOpenPositions} ${recentSettlements} ${exposureSummary} Only positions listed as open are currently open. Anything not listed in the open-position sections should be treated as settled, closed, or unavailable.`,
    );
    return this;
  }
