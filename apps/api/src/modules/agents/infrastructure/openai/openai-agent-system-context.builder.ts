...
export class OpenAiAgentSystemContextBuilder {
  ...
  addActionSemanticsSummary() {
    this.segments.push(
      `Action semantics: messaging.send_public = ${this.context.actionSemantics.sendPublicMessage} messaging.send_private = ${this.context.actionSemantics.sendPrivateMessage} transfer.request_payment = ${this.context.actionSemantics.proposeDirectTransfer} transfer.counter_payment_request = ${this.context.actionSemantics.counterDirectTransferProposal} transfer.accept_payment_request = ${this.context.actionSemantics.acceptDirectTransferProposal} transfer.reject_payment_request = ${this.context.actionSemantics.rejectDirectTransferProposal} treasury.place_with_banker = ${this.context.actionSemantics.placeFundsWithBanker} treasury.redeem_from_banker = ${this.context.actionSemantics.redeemFundsFromBanker} market.open_position = ${this.context.actionSemantics.openMarketPosition} turn.finalize = ${this.context.actionSemantics.finalizeTurn}`,
    );
    return this;
  }
  ...
  addRoleDirective() {
    ...
    switch (this.context.self.role) {
      ...
      case 'trader':
        this.segments.push(
          banker
            ? `Role economics: as the trader, you improve your outcome by deciding whether to place funds with banker ${banker.agentId}, leave funds liquid, or commit capital to listed market opportunities. Loan-style borrowing from the banker is not implemented, so do not ask the banker to fund your trading book through payment requests. If you decide to move funds into banker custody, use treasury.place_with_banker targeting banker ${banker.agentId}. If you decide to pull funds back out, use treasury.redeem_from_banker targeting banker ${banker.agentId}. When evaluating market opportunities, compare the structured facts provided in context, including expected return, downside range, signal quality, holding period, and your current balance and open positions.`
  ...
  addTurnSignal() {
    ...
    if (
      ...
      this.segments.push(
        `Current opportunity: banker ${banker.agentId} has already opened a private channel with you, so there may be value in advancing negotiation or, if you are ready to move funds into custody, directly using treasury.place_with_banker rather than a payment request.`,
      );
      return this;
    }
